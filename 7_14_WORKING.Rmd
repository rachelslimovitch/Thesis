...
```{r}
df_ELEM.temp <- expand_grid(mitigation = c(0.2, 0.35, 0.5, 0.65, 0.8), #from = 0, to = 0.99, by = 0.1), 0.9999
                            prob = 30/100000, surveillance=c(T,F), test_days=c("week", "2x_week"), test_type=c("all", "staff", "students")) %>%
  mutate(start_type = "cont", 
         n_HH = 1, #test_days = "week",
        # test_type = "all",  #WE MAY WANT TO COMBINE MULTIPLE DIFFERENT TYPES HERE
         test_sens = .8,
         total_days = 5, 
         run_specials_now = T,
         child_trans = 0.5, 
...
```{r}
#CHANGE THIS CODE: no vaccination, but we stil want to have notify, test, and test_frac
...
df_ELEM.delta =  df_ELEM.temp %>% #take df_ELEM temp and:
  expand_grid(type = c("base"), #don't need b/c default, but could add
              ...
              vax.rates = c("t50.c0", "t50.c25", "t50.c50"), #edited this
...
df<- df_ELEM %>% 
     mutate(prob = 39/100000, adult_prob = prob/((1-family_susp)+family_susp*(1-vax_eff)), 
                       ...
                       rel_trans_HH = 2*variant.attack/attack.df) 

#Call run_parallel
g = run_parallel(df_ELEM %>% 
                   mutate(prob = 39/100000, adult_prob = prob/((1-family_susp)+family_susp*(1-vax_eff)), 
                                        child_prob = prob/((1 - child.vax)+child.vax*(1-vax_eff)), 
                                        rel_trans_HH = 2*variant.attack/attack.df, n_tot = 3, sim_no = row_number()), #10 simulations per row
                 synthpop, class = class)

#add variable to df: numbers the rows
input_data<- df %>%
  mutate(sim_no=row_number())

#output_data: just class
output_data <- g %>%
  select(c(class, sim_no))

#match input data (input_data) and output data (g)
merged_data <- merge(input_data, output_data, by = "sim_no", all.x = TRUE)

```

Save dataframe:
```{r}
write.csv(merged_data, file="/Users/rachelslimovitch/Documents/22-23/Brown/Thesis and APE/Code_6_26/7_13_COVID_df2.csv", row.names=FALSE)
```
...
Make figure: 
x-axis has mitigation
y-axis has class
```{r}
library(ggplot2)
ggplot(merged_data, aes(x=mitigation, y=class)) +
  geom_point() +
  xlab("Mitigation") + 
  ylab("Class")
